...
- applied pullrequest that creates new project
  when a directory is opened on a blank workspace. 

- applied pullrequest that opens command palette
  with alternatives when completion have multiple
  choises.

- Don't prompt when creating new files or direcories.

- Code format

- Call project_manager and add new project, when
  adding directory to blank workspace.
    ...
    def open_file(self, path):
        ...
        if isdir(path):
            if is_sublime_text_2():
                ...
                if newproject:
                    sublime.active_window().run_command('project_manager', {'action': 'add_project'})
...
class iOpenerEventListener(sublime_plugin.EventListener):
    ...
    def on_query_context(self, view, key, operator, operand, match_all):
        return (key == 'i_opener'
                and iOpenerCommand.input_panel is not None
...
class iOpenerCommand(sublime_plugin.WindowCommand):
    ...
    iOpenerPathInput when the input is active,
